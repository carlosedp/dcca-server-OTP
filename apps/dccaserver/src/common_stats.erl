...
-export([get_stats/1, get_stats/2, get_stats/3, reset_stats/1, reset_stats/2]).
...
inc(Tab, Handle, Cnt) when is_atom(Cnt) ->
    inc(Tab, Handle, Cnt, 1).

inc(Tab, Handle, Cnt, Incr) when is_atom(Cnt) andalso is_integer(Incr) andalso Incr > 0 ->
    Key = {Handle, Cnt},
    do_inc(Tab, Key, Incr).
...
dec(Tab, Handle, Cnt, Decr) when is_atom(Cnt) andalso is_integer(Decr) andalso Decr < 0 ->
    Key = {Handle, Cnt},
    do_inc(Tab, Key, Decr).
...
get_handles_and_global_counters(Ets) ->
    GlobalCounters = case ets:lookup(Ets, global_counters) of
                       [{global_counters, GC}] ->
                           GC;
                       [] ->
                           []
    ...
    GlobalCounters ++ lists:sort([Handle || [Handle] <- remove_duplicates(L1, [])]).
